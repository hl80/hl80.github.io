...
function initFishes(scene, interval, speed, deadLine, width, height, hook, addScore, sharkProb) {
  ...
  update = function (dt) {
	...
	scene.physics.add.overlap(hook, fish['image'], function() { fish['dead'] = true; addScore();})
	//bubble['image'].displayWidth = 75;
	//bubble['image'].displayHeight = 75;
	fishes.push(fish)
      }
	...
	scene.physics.add.overlap(hook, shark['image'], function() {scene.scene.restart();});
	//bubble['image'].displayWidth = 75;
	//bubble['image'].displayHeight = 75;
	sharks.push(shark);
      }
...
function create() {
  ...
  upButton.setInteractive().on('pointerdown', function(pointer) {
    if(pointer.isDown)
      upDown = true;
  });
  upButton.setInteractive().on('pointerup', function() {
    upDown = false;
  });
  upButton.setInteractive().on('pointerout', function() {
    upDown = false;
  });
  ...
  downButton.setInteractive().on('pointerdown', function(pointer) {
    if(pointer.isDown)
      downDown = true;
  });
  downButton.setInteractive().on('pointerout', function() {
    downDown = false;
  });
  downButton.setInteractive().on('pointerup', function() {
    downDown = false;
  });
  ...
  var score = this.add.text(0, 0, '0', { fontFamily: 'Georgia, "Goudy Bookletter 1911", Times, serif' , fontSize: 48,});
